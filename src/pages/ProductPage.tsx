...
export default function ProductPage() {
  ...
  if (loading) {
    return (
      <section style={{ ...styles.page, background: PAGE_BG, padding: SECTION_PADDING }}>
        <div style={{ textAlign: "center", padding: "80px 20px", color: "#657447" }}>
          {(t as any).common?.loading || "Загрузка..."}
        </div>
      </section>
    );
  }
  ...
  if (error || products.length === 0) {
    return (
      <section style={{ ...styles.page, background: PAGE_BG, padding: SECTION_PADDING }}>
        <div style={{ textAlign: "center", padding: "80px 20px", color: "#a14b3a" }}>
          {error || (t as any).common?.empty || "Нет данных"}
        </div>
      </section>
    );
  }
  ...
  const productName = tr?.name || product.slug;
  const productDesc = tr?.full_desc || tr?.short_desc || "";
    ...
    <section style={{ ...styles.page, background: PAGE_BG, padding: SECTION_PADDING }}>
      ...
      <div style={styles.productGrid}>
          ...
          {product.image ? (
            <img
              ...
              style={{
                ...
                filter: "drop-shadow(0 24px 32px rgba(45,51,38,.16))",
              }}
            />
          ) : (
            <div
        ...
        <div style={styles.infoBox}>
          <button style={styles.backButton} onClick={() => navigate("catalog")}>
            ← {t.product.back}
          </button>
          ...
          <div style={styles.metaLine}>
            <span>{t.product.production}</span>
            <b>{t.product.production_val}</b>
          </div>
          <div style={styles.metaLine}>
            <span>{t.product.certs}</span>
            <b>Halal · ISO 22716</b>
          </div>
          ...
          <div style={styles.buttons}>
            {product.kaspi && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                style={{ ...styles.button, background: "#e83b45" }}
              >
                {t.product.buy_kaspi}
              </a>
            )}
            {product.wb && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                style={{ ...styles.button, background: "#7b2ff2" }}
              >
                WB
              </a>
            )}
            {product.ozon && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                style={{ ...styles.button, background: "#1268ff" }}
              >
                Ozon
              </a>
            )}
      ...
      <div
        style={{
          ...styles.relatedGrid,
          gridTemplateColumns: `repeat(${RELATED_COLUMNS}, 1fr)`,
          gap: RELATED_GAP,
        }}
      >
...
const styles: Record<string, React.CSSProperties> = {
  ...
  mediaBox: {
    ...
    alignItems: "center",
    justifyContent: "center",
    boxShadow: "0 24px 70px rgba(45,51,38,.12)",
  },
  ...
  metaLine: {
    ...
    gap: 18,
    padding: "16px 0",
    borderBottom: "1px solid rgba(101,116,71,.16)",
    color: "#25301f",
  },
