...
export default function HomePage() {
  ...
  // Подарочные наборы (категория sets); если нет — берем последние товары
  const giftImages = useMemo(() => {
    const sets = products.filter((p) => p.cat === "sets" && p.image).map((p) => p.image);
    if (sets.length > 0) return sets;
    return products.filter((p) => p.image).slice(-2).map((p) => p.image);
  }, [products]);

  const nextGift = () => {
    if (giftImages.length === 0) return;
    setGiftIndex((prev) => (prev + 1) % giftImages.length);
  };

  const prevGift = () => {
    if (giftImages.length === 0) return;
    setGiftIndex((prev) => (prev - 1 + giftImages.length) % giftImages.length);
  };

  const currentGiftImage = giftImages[giftIndex] || "";
        ...
        <div style={styles.giftGrid}>
          ...
          <div style={styles.giftImageBox}>
            ...
            {currentGiftImage ? (
              ...
              <div style={{ color: "#bfb89a", fontSize: 14 }}>Нет фото</div>
            )}
            {giftImages.length > 1 && (
              <>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  tag: { margin: "0 0 12px", color: "#3d4727", fontSize: 12, fontWeight: 900, textTransform: "uppercase", letterSpacing: 2 },
  title: { margin: 0, color: "#3d4727", fontSize: 48, lineHeight: 1.05, fontWeight: 900, letterSpacing: -1.3 },
  ...
  check: { color: "#3d4727", fontWeight: 800, fontSize: 15 },
  ...
  newBadge: { position: "absolute", top: 28, right: 28, background: "#d7b978", color: "#3d4727", height: 38, display: "flex", alignItems: "center", padding: "0 16px", borderRadius: 99, fontWeight: 900, fontSize: 12 },
