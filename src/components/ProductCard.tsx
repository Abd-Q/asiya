...
export default function ProductCard({ product }: { product: Product }) {
  ...
  // Достаём перевод имени из product.translations[locale].name с fallback на ru
  const tr = product.translations?.[locale] || product.translations?.ru || ({} as any);
  const productName = tr.name || product.slug;

  const tagText =
    product.tag === "new" ? t.catalog.tag_new : product.tag === "hit" ? t.catalog.tag_hit : "";
    ...
    >
      ...
      <div style={styles.imageBox}>
        {product.image ? (
          ...
          <div style={{ color: "#aaa", fontSize: 12 }}>Нет фото</div>
        )}
      </div>
      ...
      <div style={styles.buttons} onClick={stopClick}>
        {product.kaspi && (
          <a href={product.kaspi} target="_blank" rel="noreferrer"
             style={{ ...styles.buyButton, height: BUTTON_HEIGHT, background: "#e83b45" }}>
            Kaspi
          </a>
        )}
        {product.wb && (
          <a href={product.wb} target="_blank" rel="noreferrer"
             style={{ ...styles.buyButton, height: BUTTON_HEIGHT, background: "#7b2ff2" }}>
            WB
          </a>
        )}
        {product.ozon && (
          <a href={product.ozon} target="_blank" rel="noreferrer"
             style={{ ...styles.buyButton, height: BUTTON_HEIGHT, background: "#1268ff" }}>
            Ozon
          </a>
        )}
      </div>
